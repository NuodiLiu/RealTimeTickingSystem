...
export interface DeviceHubMethods {
  ...
  dismiss: () => void;
  ping: (payload?: { now: string }) => void;
  lockAssigned: (payload: any) => void;
  modeChanged: (payload: { mode: DeviceMode }) => void;
  unpaired: () => void;
}

export interface DeviceClientMethods {
  // Device to Server
  pong: (payload?: { now: string }) => void;
  delivered: (payload: { sessionId: string }) => void;
  lease: (payload: { deviceId: string }) => void;
  status: () => void;
  feedbackUpdate: (payload?: any) => void;
  feedbackCancelled: (payload: { sessionId: string }) => void;
}

export interface DashboardHubMethods {
  // Server to Dashboard
  caseUpdated: (payload: { id: string; status: string }) => void;
  deviceUpdated: (payload: { id: string; isBusy: boolean; isOnline: boolean }) => void;
  deviceConnected: (payload: { deviceId: string; mode: DeviceMode }) => void;
  deviceDisconnected: (payload: { deviceId: string }) => void;
}
