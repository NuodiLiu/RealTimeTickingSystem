...
exports.signalREventHandler = exports.SignalREventHandler = void 0;
const client_1 = require("./client");
const prisma_1 = require("../lib/prisma");
class SignalREventHandler {
    ...
    async handleDeviceConnect(deviceId, connectionId, mode) {
        try {
            ...
            await client_1.signalRClient.notifyDashboard({
    ...
    async handleDashboardConnect(connectionId, userId) {
        try {
            ...
            for (const device of devices) {
                ...
                const isOnline = await client_1.signalRClient.isDeviceConnected(device.id);
    ...
    // Message handlers
    async handlePong(deviceId, payload) {
    ...
    async handleDelivered(deviceId, payload) {
        console.log(`Feedback delivered on device ${deviceId}, session ${payload.sessionId}`);
        try {
            await prisma_1.prisma.feedbackSession.update({
    ...
    async handleLease(deviceId, payload) {
    ...
    async handleStatus(deviceId) {
        ...
        try {
            ...
            if (device && device.currentLock) {
                ...
                await client_1.signalRClient.assignLockToDevice(deviceId, {
    ...
    async handleFeedbackUpdate(deviceId, payload) {
        console.log(`Feedback update from device ${deviceId}`, payload);
        // Handle feedback form updates
    }
    async handleFeedbackCancelled(deviceId, payload) {
...
exports.SignalREventHandler = SignalREventHandler;
// Singleton instance
exports.signalREventHandler = new SignalREventHandler();
